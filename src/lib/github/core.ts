...
export async function fetchPublicRepos(
  ...
  if ((await store.backoff()) > Date.now())
    throw new Error("GitHub backoff active");
    ...
    const response = await fetcher(
      `https://api.github.com/users/${OWNER}/repos?type=owner&sort=full_name&per_page=100&page=${page}`,
      {
        ...
        signal: AbortSignal.timeout(
          Math.min(6000, Math.max(1, deadline - Date.now())),
        ),
      },
    );
...
export async function synchronize(
  ...
  fetcher: typeof fetch = fetch,
  removedId?: number,
): Promise<"updated" | "duplicate" | "busy"> {
  ...
  try {
    ...
    const repos = (await fetchPublicRepos(token, store, fetcher)).filter(
      (repo) => repo.id !== removedId,
    );
    if (
      !(await store.commit(
        { repos, syncedAt: new Date().toISOString() },
        before.revision,
      ))
    )
      throw new Error("Snapshot changed; retry delivery");
    await store.complete(delivery);
    return "updated";
  } finally {
    await store.release(delivery, nonce);
  }
}
