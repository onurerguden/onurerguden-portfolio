...
function Architecture() {
  ...
  useEffect(() => {
    const update = () => {
      progress.current = Math.min(
        1,
        Math.max(0, window.scrollY / (window.innerHeight * 1.5)),
      );
      invalidate();
    };
    update();
  ...
  useFrame(() => {
    ...
    if (system.current)
      system.current.rotation.y = -0.17 + progress.current * 0.17;
  });
    ...
    <group ref={system} rotation={[0, -0.17, -0.06]}>
      {[0, 1, 2, 3].map((layer) => (
        <group
          ...
          position={[0, (layer - 1.5) * 0.62, 0]}
        >
          <mesh>
            <boxGeometry args={[3.7, 0.095, 2.65]} />
            <meshPhysicalMaterial
              color={layer === 0 || layer === 3 ? "#315cd9" : "#dbe4f7"}
              metalness={0.18}
              roughness={0.24}
              clearcoat={1}
              transparent
              opacity={layer === 0 || layer === 3 ? 0.94 : 0.76}
              depthWrite={false}
            />
          </mesh>
          ...
          {layer === 1 ? (
            <mesh position={[0.65, 0.16, 0]}>
              <boxGeometry args={[0.8, 0.14, 1.36]} />
              <meshStandardMaterial
                color="#3260df"
                roughness={0.3}
                metalness={0.25}
...
function Lifecycle({ onFailure, onReady }: SceneProps) {
  ...
  return (
    <OrthographicCamera
      makeDefault
      position={[6, 5, 8]}
      zoom={Math.min(74, size.width / 6.3, size.height / 6.1)}
      near={0.1}
      far={40}
...
export default function SystemScene(props: SceneProps) {
  return (
    <Canvas
      orthographic
      camera={{ position: [6, 5, 8], zoom: 74, near: 0.1, far: 40 }}
      dpr={[1, 1.5]}
      frameloop="demand"
