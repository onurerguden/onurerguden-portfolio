...
function memoryStore(): SyncStore {
  ...
  return {
    ...
    commit: async (next, revision) => {
      ...
      snapshot = { ...next, revision: revision + 1 };
      return true;
    },
    remove: async (id) => {
      snapshot = {
        ...snapshot,
        repos: snapshot.repos.filter((repo) => repo.id !== id),
        revision: snapshot.revision + 1,
      };
    },
    acquire: async (id, nonce) => {
      if (locks.has(id)) return false;
      locks.set(id, nonce);
      return true;
    },
    release: async (id, nonce) => {
      if (locks.get(id) === nonce) locks.delete(id);
    },
    done: async (id) => done.has(id),
    complete: async (id) => {
      done.add(id);
    },
    backoff: async (until) => {
...
const repo = (id = 1, name = "example") => ({
  id,
  name,
  html_url: `https://github.com/onurerguden/${name}`,
...
describe("GitHub webhook trust boundary", () => {
  ...
  it("rejects unrelated owners and distinguishes deletion from push updates", () => {
    expect(() =>
      parseEvent(
        JSON.stringify({
          repository: { ...repo(), owner: { login: "other" } },
        }),
      ),
    ).toThrow();
    expect(
      parseEvent(JSON.stringify({ action: "deleted", repository: repo() }))
        .remove,
    ).toBe(true);
    expect(
      parseEvent(JSON.stringify({ repository: { ...repo(), private: true } }))
        .remove,
    ).toBe(true);
    expect(parseEvent(JSON.stringify({ repository: repo() })).remove).toBe(
      false,
    );
  });
});
describe("durable synchronization", () => {
    ...
    const fetcher = vi
      ...
      .mockResolvedValue(
        response([
          repo(),
          { ...repo(2), private: true },
          { ...repo(3), owner: { login: "other" } },
          repo(4, "onurerguden"),
          { ...repo(5, "fork"), fork: true },
        ]),
      );
  ...
  it("blocks concurrent processing of the same delivery", async () => {
    ...
    await started;
    expect(await synchronize(store, "token", "simultaneous", fetcher)).toBe(
      "busy",
    );
  ...
  it("does not restore a deleted ID from an eventually consistent GitHub response", async () => {
    ...
    await synchronize(
      ...
      "token",
      "removed",
      vi.fn<typeof fetch>().mockResolvedValue(response([repo()])),
      1,
    );
  ...
  it("preserves the snapshot and allows a failed delivery to be retried", async () => {
    ...
    await synchronize(
      ...
      "token",
      "retry",
      vi.fn<typeof fetch>().mockResolvedValue(response([repo(1, "renamed")])),
    );
      ...
      .mockResolvedValueOnce(
        response(
          Array.from({ length: 100 }, (_, index) =>
            repo(index + 1, `repo-${index}`),
          ),
        ),
      )
      .mockResolvedValueOnce(new Response("", { status: 500 }));
    await expect(
      synchronize(store, "token", "partial", fetcher),
    ).rejects.toThrow();
  ...
  it("follows every page and reconciles deletion after a complete read", async () => {
    ...
    const fetcher = vi
      ...
      .mockResolvedValueOnce(response([repo(101, "last")]));
  ...
  it("persists rate-limit backoff without making another request", async () => {
    ...
    await expect(
      synchronize(store, "token", "limit", fetcher),
    ).rejects.toThrow();
  ...
  it("a confirmed removal cannot be overwritten by an in-flight stale fetch", async () => {
    ...
    await expect(synchronize(store, "token", "stale", fetcher)).rejects.toThrow(
      "Snapshot changed",
    );
    expect((await store.read()).repos).toEqual([]);
  });
